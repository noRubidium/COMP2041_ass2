...
#
sub page_header {
    open F,"<","header.html" or die "file open failed";
    my @header=<F>;
    close F;
    my $header = join("",@header);
    my $log_sub = "";
    if($logged_in == 1){
    	open F,"<","loggedin.html" or die "file open failed";
    }else{
    	open F,"<","notin.html" or die "file open failed";
    }
    my @string = <F>;
    close F;
    $log_sub = join("",@string);
...
sub page_trailer {
    ...
    open F,"<","footer.html" or die "file open failed";
    my @footer=<F>;
    close F;
    $html .= join("",@footer);
    return $html;
...
sub user_display {
    ...
    open F,"<","user_display.html" or die "can't open user_display.html $!";
...
sub bleat_display{
	...
	my $result = "";
	open G,"<","single_bleat.html" or die "Fail to open single_bleat.html: $!";
	my $tmp = join "",<G>;
	close G;
	while(my $line = <F>){
		chomp $line;
...
sub info_panel{
    ...
    #open user_display.html
    open F,"<","info_panel.html" or die "can't open user_display.html $!";
    my $user_display = join '', <F>;
    close F;
